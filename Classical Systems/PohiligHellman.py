...
def ShanksAlgorithm(alpha, beta, n):
    """Return s s.t. beta ≡ alpha^(s) (mod n)"""
    m = int(ceil(sqrt(n - 1)))
    a = pow(alpha, m, n)
...
def CongruencePair(g, h, p, q, e, e1, e2):
    """Return pair (s, q ** e) which represents one congruence"""
    alphaInverse = ModularInverse(e1, p)
    x = 0 # s = x_{0} + x_{1} * q + x_{2} * q^{2} + ... + x_{e - 1} * q^{e - 1}
    for i in range(1, e + 1):
        a = pow(round(e1), round(q ** (e - 1)), p)
        b = pow(round(e2 * (alphaInverse ** x)), round(q ** (e - i), p))
...
def PohlingHellman(g, h, p):
    ...
    print("\n")
    print("-"*90)
    print(" Solving %d ≡ %d^s (mod %d)" % (g, h, p))
    print("-"*90)
    PrintFormated("q", "e", "g^((p-1)/q^e)", "h^((p-1)/q^e)", "Solve (g^((p-1)/q^e))^s = h^((p-1)/q^e) for s")
    ...
    for i in range(len(CountOccurencesList)):
        ...
        e3 = CongruenceList[len(CongruenceList) - 1][0] % CongruenceList[len(CongruenceList) - 1][1] # e3 = (g^((p-1)/q^e))^s
        e4 = CongruenceList[len(CongruenceList) - 1][1] # e4 = h^((p-1)/q^e)
        PrintFormated(CountOccurencesList[i][0], CountOccurencesList[i][1], e1, e2, "s ≡ %2d (mod %2d)" % (e3, e4))

    # Solve the system of congruences
    print(" Solution s = %d" % ChineseRemainder(CongruenceList))
    print("-"*90)
    print("\n")
    ...
    while True:
        ...
        if not err:
            try:
                ...
                print(" The congruence %d ≡ %d^s (mod %d) has no solution " % (h, g, p))
                print("-" * 90)
                print("\n")